...
describe "Vidibus::Service::ConnectorApp" do
  ...
  let(:endpoint) { 'http://manager.local/connector' }
  ...
  describe "POST requests" do
    ...
    context "with Connector params" do
      ...
      it "should accept any value as key" do
        post endpoint, {"something" => connector_params}
        last_response.status.should eql(201)
      end
    ...
    context "with existing Connector" do
      ...
      it "should fail if params for this service are invalid" do
        post endpoint, {this_uuid => this_params.except(:function)}
        last_response.status.should eql(400)
        last_response.body.should match("Setting up this service failed:")
      end

      it "should fail if a secret is given" do
        post endpoint, {this_uuid => this_params.merge(:secret => "something")}
        last_response.status.should eql(400)
      ...
      it "should fail unless a nonce is given" do
        post endpoint, {this_uuid => this_params}
        last_response.status.should eql(400)
      ...
      it "should request a secret for this Service" do
        ...
        post endpoint, {this_uuid => this_params.merge(:nonce => nonce)}
      end

      it "should fail if sign cannot be validated with nonce" do
        stub_secret_request!
        post endpoint, {this_uuid => this_params.merge(:nonce => "invalid")}
        last_response.status.should eql(400)
      ...
      it "should set up this Service with valid params" do
        stub_secret_request!
        post endpoint, {connector_uuid => this_params.merge(:nonce => nonce)}
      ...
      it "should fail if service data is invalid" do
        ...
        post(endpoint, {this_uuid => params})
        last_response.status.should eql(400)
        last_response.body.should match("Setting up this service failed:")
      end
    end
  ...
  describe "PUT requests" do
    ...
    it "should update an existing service without realm_uuid" do
      ...
      signed_request(:put, endpoint, {connector.uuid => {:url => url}})
      last_response.status.should eql(200)
    ...
    it "should update an existing service with given realm_uuid" do
      ...
      signed_request(:put, endpoint, {uploader.uuid => {:url => url, :realm_uuid => uploader.realm_uuid}})
      last_response.status.should eql(200)
    ...
    it "should not update existing services with a different realm_uuid" do
      ...
      signed_request(:put, endpoint, {uploader.uuid => {:url => "http://newuploader.local", :realm_uuid => uploader.realm_uuid}})
      last_response.status.should eql(200)
    ...
    it "should fail if no uuid is given" do
      this and connector
      url = "http://newconnector.local"
      signed_request(:put, endpoint, {:url => url})
      last_response.status.should eql(400)
    ...
    it "should fail if an invalid uuid is given" do
      this and connector
      url = "http://newconnector.local"
      signed_request(:put, endpoint, {"c0861d609247012d0a8b58b035f038ab" => {:url => url}})
      last_response.status.should eql(400)
    ...
    it "should fail if invalid data is given" do
      this and connector
      url = "http://newconnector.local"
      signed_request(:put, endpoint, {connector.uuid => {:secret => "not allowed"}})
      last_response.status.should eql(400)
  ...
  describe "DELETE requests" do
    ...
    it "should fail if list of UUIDs is not given" do
      this and connector
      signed_request(:delete, endpoint, {})
      last_response.status.should eql(400)
    ...
    it "should not care if any given UUID is invalid" do
      this and connector
      signed_request(:delete, endpoint, {:uuids =>["invalid", connector_uuid]})
      last_response.status.should eql(200)
    end
  end
end
